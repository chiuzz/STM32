...
PMODULEBASE CreateModuleServer()
{
    ...
    MsgNotiflyPtr pFun = ServerMsgNotify;
    ...
    pData->m_pSysData = (PCONFIGDATA)malloc(sizeof(CONFIGDATA));
...
//  printf("sizeof(MODULESERVER) %d\n",(int)sizeof(MODULESERVER));
    ...
    pModServer->pMsgNotifyProcess = MsgNotifyProcess;//实现消息调用
    //************************以上模块接口必须赋值******************************

    //************************初始化消息队列************************************

    pData->m_pMsgQueue = MsgQueueCreate(MSGQUEUE_ID,pFun, pModServer->m_pBaseData);
