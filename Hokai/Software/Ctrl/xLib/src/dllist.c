...
//	ASSERT (Item != NULL);
	...
	p = DLCreate (Tag, Object, Size);		//创建了一个无指向链表
...
//          printf("%s:%d Change prev pointer.\n", __FILE__, __LINE__);
		}
		if (Item->Next != NULL)
		{
			Item->Next->Prev = Item->Prev;
//          printf("%s:%d Change next pointer.\n", __FILE__, __LINE__);
		}
		Item->Prev = Item->Next = NULL;
//      printf("%s:%d Change Self pointer.\n", __FILE__, __LINE__);
	}
...
void DLDelete (DLLIST * Item)
{
	if (Item != NULL)
	{
		...
		{
			free (Item->Object);
//          printf("%s:%d Free Object.\n", __FILE__, __LINE__);
		}
		free (Item);
//      printf("%s:%d Free Item.\n", __FILE__, __LINE__);
	}
}
...
int DLWalk (DLLIST * List,
			DLLIST ** addr, int (*Func) (int, void *, void *), void *Args)
{
	...
	if (List != NULL)
	{
		for (ThisItem = DLGetFirst (List);		//找表头
			 0 == Result && ThisItem != NULL;
				ThisItem = ThisItem->Next)				//下一个表
		{
